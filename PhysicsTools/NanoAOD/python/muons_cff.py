...
isoForMu = cms.EDProducer("MuonIsoValueMapProducer",
    ...
    ## for Ghent tZqTTV LeptonMVA (should not affect any other variables)
...
muonMVAGhent = cms.EDProducer("MuonBaseMVAValueMapProducer",
    ...
    variables = cms.PSet(
        ...
        trackMultClosestJet = cms.string("?userCand('jetForLepJetVar').isNonnull()&&(userFloat('ptRel')!=0.)?userFloat('jetNDauChargedMVASel'):1"),
        ...
        ptRatio = cms.string("?userCand('jetForLepJetVar').isNonnull()?(?userFloat('ptRel')!=0.?min(userFloat('ptRatio'),1.5):1./((userCand('jetForLepJetVar').p4().pt()/pt)-(1./userCand('jetForLepJetVar').jecFactor('L1FastJet'))+1)):1"),
