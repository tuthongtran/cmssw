...
SiStripApvSimulationParametersBuilder::SiStripApvSimulationParametersBuilder(const edm::ParameterSet& conf)
...
SiStripApvSimulationParameters::LayerParameters SiStripApvSimulationParametersBuilder::makeLayerParameters(
...
void SiStripApvSimulationParametersBuilder::analyze(const edm::Event&, const edm::EventSetup& /*evtSetup*/) {
  ...
  */
  ...
  for (unsigned int i{0}; i != baselineFiles_TIB_.size(); ++i) {
    if ( ! obj->putTIB(i + 1, makeLayerParameters(baselineFiles_TIB_[i].fullPath())) ) {
  ...
  for (unsigned int i{0}; i != baselineFiles_TOB_.size(); ++i) {
    if ( ! obj->putTOB(i + 1, makeLayerParameters(baselineFiles_TOB_[i].fullPath())) ) {
