...
inline int
SiStripGainsCalibTreeWorker::statCollectionFromMode(const char* tag) const
{
...
//********************************************************************************//
void
SiStripGainsCalibTreeWorker::dqmBeginRun(edm::Run const& run, edm::EventSetup const& iSetup, APVGain::APVGainHistograms & histograms) const {
...
void
SiStripGainsCalibTreeWorker::dqmAnalyze(edm::Event const& iEvent, edm::EventSetup const& iSetup, APVGain::APVGainHistograms const& histograms) const
{
  if ( ! hasProcessed_ ) {
    ...
    for (const auto &elem : theTopologyMap){
      LogDebug("SiStripGainsCalibTreeWorker") << elem.first << " - " << elem.second.m_string << " " << elem.second.m_subdetectorId << " " << elem.second.m_subdetectorSide << " " << elem.second.m_subdetectorPlane << std::endl;
    }
    ...
    for ( Long64_t i{0}; i != nEntries; ++i ) {
      if ( (i % treeProgStep) == 0 ) { printf("."); fflush(stdout); }
      tree.GetEntry(i);
      ...
      for(unsigned int i=0;i<charge->size();i++){
        ...
        LogDebug("SiStripGainsCalibTreeWorker") <<" full charge "<< mCharge1
                                          <<" remove G2 "<< mCharge2
                                          <<" remove G1 "<< mCharge3
...
//********************************************************************************//
void
SiStripGainsCalibTreeWorker::beginJob()
{
}
...
void
SiStripGainsCalibTreeWorker::checkBookAPVColls(const TrackerGeometry *bareTkGeomPtr,APVGain::APVGainHistograms & histograms) const
{
...
//********************************************************************************//
void
SiStripGainsCalibTreeWorker::endJob()
{
}

//********************************************************************************//
void
SiStripGainsCalibTreeWorker::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
//********************************************************************************//
void
SiStripGainsCalibTreeWorker::bookHistograms(DQMStore::ConcurrentBooker & ibooker, edm::Run const& run, edm::EventSetup const& setup, APVGain::APVGainHistograms & histograms) const {

  ibooker.cd();
  std::string dqm_dir = m_DQMdir;
  const char* tag = dqm_tag_[statCollectionFromMode(m_calibrationMode.c_str())].c_str();

  edm::LogInfo("SiStripGainsCalibTreeWorker") << "Setting " << dqm_dir << " in DQM and booking histograms for tag "
					<< tag << std::endl;
