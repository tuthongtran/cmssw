...
namespace {
  ...
  public:
    ...
    StripByStripAdder(StripClusterizerAlgorithm& clusterizer,
      ...
      : clusterizer_(clusterizer), record_(record) {}

    StripByStripAdder& operator= ( SiStripDigi digi )
    {
      clusterizer_.stripByStripAdd(digi.strip(), digi.adc(), record_);
      return *this;
    }
  ...
  private:
    ...
    StripClusterizerAlgorithm::output_t::FastFiller& record_;
  };
}
...
try {
  ...
  for (auto const conn : clusterizer.currentConnection()) {
    ...
    if likely( ( mode > sistrip::READOUT_MODE_VIRGIN_RAW ) && ( mode < sistrip::READOUT_MODE_SPY ) && ( mode != sistrip::READOUT_MODE_PROC_RAW ) ) {
      // ZS modes
      try {
        auto perStripAdder = StripByStripAdder(clusterizer, record);
      ...
      for ( const auto digi : zsdigis ) {
        clusterizer.stripByStripAdd(digi.strip(), digi.adc(), record);
      }
