...
class SiStripAPVRestorer {
 ...
 public:
  ...
  uint16_t  inspect(uint32_t detId, uint16_t firstAPV, const std::vector<int16_t>& digis, const std::vector< std::pair<short,float> >& vmedians);
  void     restore(uint16_t firstAPV, std::vector<int16_t>&);
  ...
  uint16_t  InspectAndRestore(uint32_t detId, uint16_t firstAPV, const std::vector<int16_t>& rawDigisPedSubtracted, std::vector<int16_t>& processedRawDigi, const std::vector< std::pair<short,float> >& vmedians);
 ...
 private:
  ...
  template<typename T >uint16_t NullInspect(uint16_t firstAPV, const std::vector<T>& digis);
  template<typename T >uint16_t AbnormalBaselineInspect(uint16_t firstAPV, const std::vector<T>& digis);
  template<typename T >uint16_t BaselineFollowerInspect(uint16_t firstAPV, const std::vector<T>& digis);
  template<typename T >uint16_t BaselineAndSaturationInspect(uint16_t firstAPV, const std::vector<T>& digis);
  template<typename T >uint16_t ForceRestoreInspect(uint16_t firstAPV, const std::vector<T>& digis);
  template<typename T >uint16_t HybridFormatInspect(uint16_t firstAPV, const std::vector<T>& digis);

  void FlatRestore(uint16_t APVn, uint16_t firstAPV, std::vector<int16_t>& digis);
  bool CheckBaseline(const std::vector<int16_t> & baseline) const;
  void BaselineFollowerRestore(uint16_t APVn, uint16_t firstAPV, float median, std::vector<int16_t>& digis);
  void DerivativeFollowerRestore(uint16_t APV, uint16_t firstAPV, std::vector<int16_t>& digis);

  void BaselineFollower(DigiMap&, std::vector<int16_t>& baseline, float median);
  bool FlatRegionsFinder(const std::vector<int16_t>&, DigiMap&, uint16_t APVn);

  void BaselineCleaner(const std::vector<int16_t>&, DigiMap&, uint16_t APVn);
  void Cleaner_MonotonyChecker(DigiMap&);
  void Cleaner_HighSlopeChecker(DigiMap&);
  void Cleaner_LocalMinimumAdder(const std::vector<int16_t>&, DigiMap&, uint16_t APVn);
